...
### Arreglos y Ajustes Visuales
- **Botones de herramientas:** Se ha corregido el color de los botones del menú de inicio (Buscar, Base de datos local, Sonido, Temas, Insertar Proyecto y el botón de Instrucciones), **así como los botones del menú superior dentro de cada proyecto** (Inicio, Compartir proyecto y Guardar). Ahora todos se adaptan dinámicamente al tema elegido utilizando el color principal de la paleta actual (más suave y menos intenso que en pruebas anteriores), en lugar de quedarse marrones de forma estática. Esto mejora la legibilidad con texto en blanco y aplica sombras a juego.
