...
- **Rediseño de Botonera Superior (Móvil):** Se ha optimizado la cuadrícula de los botones principales en la versión móvil para que se muestren simétricos en dos filas perfectas. En la primera fila se prioriza el buscador (más largo), se ha acortado el texto de "Base de datos local" a "BD local", y se ha subido el botón del manual (📖) al extremo derecho. En la segunda fila, los botones de "Sonido", "Temas" y "Nuevo" (antes "Insertar Proyecto") ocupan ahora exactamente el mismo espacio (33% cada uno) permitiendo que sus textos se lean por completo sin cortarse.
...
- **Manual de Instrucciones (Actualización):** Se ha insertado un nuevo punto "2. Importante antes de empezar a usar la aplicación" en el índice. En él se explica detalladamente, paso a paso y con iconos ilustrativos (💾, 📤, 📥), la importancia de realizar copias de seguridad y cómo exportar e importar los datos de la app. Todos los puntos posteriores del manual han sido renumerados. Además, se ha reescrito la sección "3. Pantalla de inicio" con una descripción más detallada y actualizada de todos los botones y funciones principales (como el Manual, Nuevo, Proyectos, Filtros, Calculadora y botones inferiores).
...
- **Correcciones Visuales:** Se ha solucionado un error visual donde el texto flotante de "Guarda tus datos regularmente" y la etiqueta de la versión se superponían (y atravesaban) por encima de las ventanas emergentes como el manual de instrucciones.
- **Arranque Local (Fix):** Se ha eliminado una duplicidad antigua en el código de registro del Service Worker que seguía lanzando un aviso en la consola al abrir la aplicación de manera local mediante el protocolo `file://`.
...
- **Corrección en el Manual:** Se ha corregido un texto erróneo en el manual de instrucciones. Se ha aclarado que el botón "Base de datos local" de la pantalla de inicio no sirve para hacer copias de seguridad, sino que es estrictamente un indicador visual de estado (punto verde = conectado y funcionando; punto rojo = sin conexión).
- **Logo de la Calculadora:** Se ha solucionado un error visual por el cual el botón de firma (EliBi) dentro de la calculadora mostraba un icono de imagen rota en lugar del logotipo. Se ha vuelto a incrustar la imagen en formato base64 directamente en el código para asegurar que siempre cargue correctamente incluso sin conexión a internet.
